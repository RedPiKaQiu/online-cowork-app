...
export function BoardClient({ snapshot, token }: { snapshot: ProjectSnapshot; token: string }) {
  ...
  async function mutate(id: string, optimistic: () => void, request: () => Promise<void>) {
    ...
    try { await request(); return true } catch (error) {
  ...
  async function addTask(input: { title: string; status: ColumnId; assigneeId: string | null }) {
    if (pending("add")) return false
    const localId = `local-${crypto.randomUUID()}`
    const assigneeId = input.status === "box" ? null : input.assigneeId
    const task = { id: localId, title: input.title, description: "", assigneeId, version: 1 }
    return mutate("add", () => dispatch({ type: "task-upsert", task, status: input.status }), async () => {
      const response = await projectBoardApi.createTask(token, { ...input, assigneeId })
      dispatch({ type: "task-upsert", task: response.task, status: input.status, previousId: localId })
    })
  }
